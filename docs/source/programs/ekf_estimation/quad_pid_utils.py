...
quad_pid_utils.py
=================
Supporting module for the Quadcopter Cascade PID.
...
Contents:
  ...
  - InitializeControllers()               : instantiate the 3 PID loops + allocator
...
It additionally keeps the two
pieces of interface that ekf.py depends on:
  - InitializeControllers(controller, quad) is exposed as a standalone
    function (not inlined into run_fig8_pid).
  - run_fig8_pid() returns (quad, rotor_history) so the EKF can replay
    ACTUAL rotor speeds instead of the constant hover approximation.
...
def run_fig8_pid(config):
    ...
    # ── pre-allocate rotor history for the EKF ──
    ...
    for t in np.arange(0, tf, dt):
        ...
        # ── store actual rotor speeds at this timestep ──
        if step < N_steps:
